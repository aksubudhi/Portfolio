...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Removed the click event listener that closes the side box when clicking outside of it
  // document.addEventListener('click', function (event) {
  //   if (!sideBox.contains(event.target) && !openBoxButton.contains(event.target)) {
  //     sideBox.classList.remove('active');
  //     sideBox.classList.add('hidden');
  //   }
  // });
  ...
  switch (activeCorner) {
    ...
    case "bottom-right":
      if (window.innerWidth <= 600) {
        ...
        brContent.style.width = "15rem";
        brContent.style.height = "0";
        brContent.style.top = "40vh";
...
// Play closing animation function
